...
void __cu2cl_Init_transpose_cu() {
    ...
    if(err != CL_SUCCESS){
        ...
        size_t logSize;
        err = clGetProgramBuildInfo(__cu2cl_Program_transpose_cu, &__cu2cl_Device, CL_PROGRAM_BUILD_LOG, 0, nullptr, &logSize);
        printf("clGetProgramBuildInfo : %s\n", getErrorString(err));
        buildLog.resize(logSize);
        clGetProgramBuildInfo(__cu2cl_Program_transpose_cu, __cu2cl_Device, CL_PROGRAM_BUILD_LOG, logSize, &buildLog[0], nullptr);
        std::cout << &buildLog[0] << '\n';
    }
