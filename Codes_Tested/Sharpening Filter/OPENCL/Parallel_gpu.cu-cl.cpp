...
void __cu2cl_Init_Parallel_gpu_cu() {
    ...
    //printf("clCreateProgramWithBinary for Parallel_gpu.cu-cl.cl: %s\n", getErrorString(err));
    #else
    ...
    if(err != CL_SUCCESS){
        ...
        size_t logSize;
        err = clGetProgramBuildInfo(__cu2cl_Program_Parallel_gpu_cu, __cu2cl_Device, CL_PROGRAM_BUILD_LOG, 0, NULL, &logSize);
