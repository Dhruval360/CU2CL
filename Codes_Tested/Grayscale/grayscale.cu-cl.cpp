...
cl_mem        d_rgbaImage__;cl_mem  d_greyImage__;
...
void preProcess(cl_uchar4 **inputImage, cl_uchar  **greyImage, cl_mem  d_rgbaImage, cl_mem  d_greyImage,const std::string &filename) {
  ...
  *inputImage = (cl_uchar4 *)imageRGBA.ptr<cl_uchar>(0);
  *greyImage  = imageGrey.ptr<cl_uchar>(0);
...
/*CU2CL Note -- Rewriting single decl*/
  d_greyImage = clCreateBuffer(__cu2cl_Context, CL_MEM_READ_WRITE, sizeof(uchar) * numPixels, NULL, &err);
//printf("clCreateBuffer for device variable *d_greyImage is: %s\n", getErrorString(err));
  __cu2cl_Memset(d_greyImage, 0, numPixels * sizeof(uchar)); //make sure no memory is left laying around
...
void postProcess(const std::string& output_file) {
  ...
  err = clEnqueueReadBuffer(__cu2cl_CommandQueue, d_greyImage__, CL_TRUE, 0, sizeof(uchar) * numPixels, imageGrey.ptr<uchar>(0), 0, NULL, NULL);
...
void your_rgba_to_greyscale(const cl_uchar4 * const h_rgbaImage,cl_mem const d_rgbaImage,
                            cl_mem const d_greyImage, size_t numRows, size_t numCols)
{
  const int thread = 16;
...
/*CU2CL Note -- Fast-tracked dim3 type without cast*/
  err = clSetKernelArg(__cu2cl_Kernel_rgba_to_greyscale, 0, sizeof(const cl_uchar4 *), &d_rgbaImage);
...
err = clSetKernelArg(__cu2cl_Kernel_rgba_to_greyscale, 1, sizeof(uchar *), &d_greyImage);
...
__cu2cl_Init();
  ...
  cl_uchar4        *h_rgbaImage;cl_mem d_rgbaImage;
  uchar  *h_greyImage;cl_mem d_greyImage;
