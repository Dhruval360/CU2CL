...
", getErrorString(err));    #endif
    ...
    if(err != CL_SUCCESS){        std::vector<char> buildLogUtil;        size_t logSizeUtil;        err = clGetProgramBuildInfo(__cu2cl_Util_Program, &__cu2cl_Device, CL_PROGRAM_BUILD_LOG, 0, nullptr, &logSizeUtil);        printf("clGetProgramBuildInfo : %s\n", getErrorString(err));        buildLogUtil.resize(logSizeUtil);        clGetProgramBuildInfo(__cu2cl_Util_Program, __cu2cl_Device, CL_PROGRAM_BUILD_LOG, logSizeUtil, &buildLogUtil[0], nullptr);        std::cout << &buildLogUtil[0] << '
