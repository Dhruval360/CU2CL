...
void __cu2cl_Init_redEYECPU_cu() {
    ...
    __cu2cl_Program_redEYECPU_cu = clCreateProgramWithBinary(__cu2cl_Context, 1, &__cu2cl_Device, &progLen, (const unsigned char **)&progSrc, NULL, NULL);
    #else
    progLen = __cu2cl_LoadProgramSource("redEYECPU.cu-cl.cl", &progSrc);
    __cu2cl_Program_redEYECPU_cu = clCreateProgramWithSource(__cu2cl_Context, 1, &progSrc, &progLen, NULL);
    #endif
    free((void *) progSrc);
    clBuildProgram(__cu2cl_Program_redEYECPU_cu, 1, &__cu2cl_Device, "-I . ", NULL, NULL);
    __cu2cl_Kernel_naive_normalized_cross_correlation = clCreateKernel(__cu2cl_Program_redEYECPU_cu, "naive_normalized_cross_correlation", NULL);
    __cu2cl_Kernel_remove_redness_from_coordinates = clCreateKernel(__cu2cl_Program_redEYECPU_cu, "remove_redness_from_coordinates", NULL);
}
...
/*CU2CL Unsupported -- Unsupported CUDA call: cudaGetLastError*/
    clFinish(__cu2cl_CommandQueue); checkCudaErrors(cudaGetLastError());
