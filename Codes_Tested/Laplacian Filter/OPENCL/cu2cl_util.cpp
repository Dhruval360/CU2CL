...
extern cl_kernel __cu2cl_Kernel_laplacianFilter;
extern cl_program __cu2cl_Program_Laplacian_Filter_cu;
extern cl_int err;
const char *progSrc;
size_t progLen;
...
//printf("Creation of main command queue: %s\n", getErrorString(err));
    __cu2cl_Init_Laplacian_Filter_cu();
}

void __cu2cl_Cleanup() {
    __cu2cl_Cleanup_Laplacian_Filter_cu();
    clReleaseCommandQueue(__cu2cl_CommandQueue);
    clReleaseContext(__cu2cl_Context);
}
