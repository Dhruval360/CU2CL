...
cl_program __cu2cl_Program_median_cu;
...
void __cu2cl_Cleanup_median_cu() {
    clReleaseKernel(__cu2cl_Kernel_medianFilter);
    clReleaseProgram(__cu2cl_Program_median_cu);
}
void __cu2cl_Init_median_cu() {
    #ifdef WITH_ALTERA
    progLen = __cu2cl_LoadProgramSource("median_cu_cl.aocx", &progSrc);
    __cu2cl_Program_median_cu = clCreateProgramWithBinary(__cu2cl_Context, 1, &__cu2cl_Device, &progLen, (const unsigned char **)&progSrc, NULL, NULL);
    #else
    progLen = __cu2cl_LoadProgramSource("median.cu-cl.cl", &progSrc);
    __cu2cl_Program_median_cu = clCreateProgramWithSource(__cu2cl_Context, 1, &progSrc, &progLen, NULL);
    #endif
    free((void *) progSrc);
    clBuildProgram(__cu2cl_Program_median_cu, 1, &__cu2cl_Device, "-I . ", NULL, NULL);
    __cu2cl_Kernel_medianFilter = clCreateKernel(__cu2cl_Program_median_cu, "medianFilter", NULL);
}
...
/*CU2CL Note -- Fast-tracked dim3 type without cast*/
        clSetKernelArg(__cu2cl_Kernel_medianFilter, 0, sizeof(cl_mem), &d_input);
clSetKernelArg(__cu2cl_Kernel_medianFilter, 1, sizeof(cl_mem), &d_output);
...
clEnqueueNDRangeKernel(__cu2cl_CommandQueue, __cu2cl_Kernel_medianFilter, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
