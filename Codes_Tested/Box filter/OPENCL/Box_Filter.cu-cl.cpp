...
cl_kernel __cu2cl_Kernel_boxFilter;
cl_program __cu2cl_Program_Box_Filter_cu;
cl_int err;
...
void __cu2cl_Cleanup_Box_Filter_cu() {
    clReleaseKernel(__cu2cl_Kernel_boxFilter);
    clReleaseProgram(__cu2cl_Program_Box_Filter_cu);
}
void __cu2cl_Init_Box_Filter_cu() {
    #ifdef WITH_ALTERA
    progLen = __cu2cl_LoadProgramSource("Box_Filter_cu_cl.aocx", &progSrc);
    __cu2cl_Program_Box_Filter_cu = clCreateProgramWithBinary(__cu2cl_Context, 1, &__cu2cl_Device, &progLen, (const unsigned char **)&progSrc, NULL, &err);
    //printf("clCreateProgramWithBinary for Box_Filter.cu-cl.cl: %s\n", getErrorString(err));
    #else
    progLen = __cu2cl_LoadProgramSource("Box_Filter.cu-cl.cl", &progSrc);
    __cu2cl_Program_Box_Filter_cu = clCreateProgramWithSource(__cu2cl_Context, 1, &progSrc, &progLen, &err);
    //printf("clCreateProgramWithSource for Box_Filter.cu-cl.cl: %s\n", getErrorString(err));
    #endif
    free((void *) progSrc);
    err = clBuildProgram(__cu2cl_Program_Box_Filter_cu, 1, &__cu2cl_Device, "-I . ", NULL, NULL);
    /*printf("clBuildProgram : %s\n", getErrorString(err)); //Uncomment this line to access the error string of the error code returned by clBuildProgram*/
    if(err != CL_SUCCESS){
        std::vector<char> buildLog;
        size_t logSize;
        err = clGetProgramBuildInfo(__cu2cl_Program_Box_Filter_cu, __cu2cl_Device, CL_PROGRAM_BUILD_LOG, 0, NULL, &logSize);
        printf("clGetProgramBuildInfo : %s\n", getErrorString(err));
        buildLog.resize(logSize);
        clGetProgramBuildInfo(__cu2cl_Program_Box_Filter_cu, __cu2cl_Device, CL_PROGRAM_BUILD_LOG, logSize, &buildLog[0], NULL);
        printf("%s\n", &buildLog[0]);
    }
    __cu2cl_Kernel_boxFilter = clCreateKernel(__cu2cl_Program_Box_Filter_cu, "boxFilter", &err);
    /*printf("__cu2cl_Kernel_boxFilter creation: %s\n", getErrorString(err)); // Uncomment this line to get error string for the error code returned by clCreateKernel while creating the Kernel: boxFilter*/
}
...
void boxFilter_GPU_wrapper(const cv::Mat& input, cv::Mat& output);
...
/*CU2CL Note -- Fast-tracked dim3 type without cast*/
        err = clSetKernelArg(__cu2cl_Kernel_boxFilter, 0, sizeof(cl_mem), &d_input);
/*printf("clSetKernelArg for argument 0 of kernel __cu2cl_Kernel_boxFilter: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
err = clSetKernelArg(__cu2cl_Kernel_boxFilter, 1, sizeof(cl_mem), &d_output);
/*printf("clSetKernelArg for argument 1 of kernel __cu2cl_Kernel_boxFilter: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
err = clSetKernelArg(__cu2cl_Kernel_boxFilter, 2, sizeof(unsigned int), &output.cols);
/*printf("clSetKernelArg for argument 2 of kernel __cu2cl_Kernel_boxFilter: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
err = clSetKernelArg(__cu2cl_Kernel_boxFilter, 3, sizeof(unsigned int), &output.rows);
/*printf("clSetKernelArg for argument 3 of kernel __cu2cl_Kernel_boxFilter: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
err = clSetKernelArg(__cu2cl_Kernel_boxFilter, 4, sizeof(int), &channel);
/*printf("clSetKernelArg for argument 4 of kernel __cu2cl_Kernel_boxFilter: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
...
err = clEnqueueNDRangeKernel(__cu2cl_CommandQueue, __cu2cl_Kernel_boxFilter, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
//printf("clEnqueueNDRangeKernel for the kernel __cu2cl_Kernel_boxFilter: %s\n", getErrorString(err));
