...
void __cu2cl_Init_sxpy_cu() {
    ...
    //printf("clCreateProgramWithBinary for sxpy.cu-cl.cl: %s\n", getErrorString(err));
    #else
    ...
    if(err != CL_SUCCESS){
        ...
        size_t logSize;
        err = clGetProgramBuildInfo(__cu2cl_Program_sxpy_cu, __cu2cl_Device, CL_PROGRAM_BUILD_LOG, 0, NULL, &logSize);
...
//printf("clCreateBuffer for device variable d_x: %s\n", getErrorString(err)); 
  d_y = clCreateBuffer(__cu2cl_Context, CL_MEM_READ_WRITE, N*sizeof(float), NULL, &err);
//printf("clCreateBuffer for device variable d_y: %s\n", getErrorString(err));
...
/*printf("clSetKernelArg for argument 0 of kernel __cu2cl_Kernel_saxpy: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
float __cu2cl_Kernel_saxpy_temp_arg_10 = 2.0f;
err = clSetKernelArg(__cu2cl_Kernel_saxpy, 1, sizeof(float), &__cu2cl_Kernel_saxpy_temp_arg_10);
/*printf("clSetKernelArg for argument 1 of kernel __cu2cl_Kernel_saxpy: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
err = clSetKernelArg(__cu2cl_Kernel_saxpy, 2, sizeof(cl_mem), &d_x);
/*printf("clSetKernelArg for argument 2 of kernel __cu2cl_Kernel_saxpy: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
err = clSetKernelArg(__cu2cl_Kernel_saxpy, 3, sizeof(cl_mem), &d_y);
/*printf("clSetKernelArg for argument 3 of kernel __cu2cl_Kernel_saxpy: %s\n", getErrorString(err));//Uncomment this for getting error string of the error code returned by clSetKernelArg*/
localWorkSize[0] = 256;
