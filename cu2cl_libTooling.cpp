...
#define CL_COMMAND_QUEUE_QUERY \
    ...
    "   //printf(\"clEnqueueMarker return from __cu2cl_CommandQueueQuery: %s\\n\", getErrorString(ret));\n"\
...
private:
    ...
    bool RewriteCUDACall(CallExpr *cudaCall, std::string &newExpr) {
        ...
        //Stream Management
            ...
            newPStream = newPStream[0] == '&' ? newPStream.substr(1) : ('*' + newPStream);
            newExpr = newPStream + " = clCreateCommandQueue(__cu2cl_Context, __cu2cl_Device, CL_QUEUE_PROFILING_ENABLE, &err);\n//printf(\"clCreateCommandQueue for stream " + newPStream + " is: %s\\n\", getErrorString(err))";
        }
        ...
        // call, then separate off the rest of the math as the offset)
            ...
            if (enumString == "cudaMemcpyHostToHost") {
                ...
                VarDecl *var = dyn_cast<VarDecl>(dr->getDecl());
                llvm::StringRef varName = var->getName();
                newExpr = "err = clEnqueueWriteBuffer(" + newStream + ", " + newDst + ", CL_FALSE, 0, " + newCount + ", " + newSrc + ", 0, NULL, NULL);\n//printf(\"Memory copy from host variable " + newSrc + " to device variable " + newDst + " in stream " + newStream + ": %s\\n\", getErrorString(err))";
            }
            else if (enumString == "cudaMemcpyDeviceToHost") {
                ...
                VarDecl *var = dyn_cast<VarDecl>(dr->getDecl());
                llvm::StringRef varName = var->getName();
                newExpr = "err = clEnqueueReadBuffer(" + newStream + ", " + newSrc + ", CL_FALSE, 0, " + newCount + ", " + newDst + ", 0, NULL, NULL);\n//printf(\"Memory copy from device variable " + newDst + " to host variable " + newSrc + " in stream " + newStream + ": %s\\n\", getErrorString(err))";
            }
            else if (enumString == "cudaMemcpyDeviceToDevice") {
                //clEnqueueCopyBuffer
                newExpr = "err = clEnqueueCopyBuffer(__cu2cl_CommandQueue, " + newSrc + ", " + newDst + ", 0, 0, " + newCount + ", 0, NULL, NULL);\n//printf(\"Memory copy from device variable " + newSrc + " to device variable " + newDst + " in stream " + newStream + ": %s\\n\", getErrorString(err))";
            }
            else {
                emitCU2CLDiagnostic(SM, cudaCall->getLocStart(), "CU2CL Unsupported", "Unsupported cudaMemcpyKind: " + enumString, &HostReplace);
            }
    ...
    std::string RewriteCUDAKernelCall(CUDAKernelCallExpr *kernelCall) {
        ...
        for (unsigned i = 0; i < kernelCall->getNumArgs(); i++) {
            ...
            //If the arg is just a declared variable, simply pass its address
                ...
                args << "err = clSetKernelArg(" << kernelName << ", " << i << ", sizeof(";
...
//                RewriteType(origTL, "cl_mem", HostReplace);
        ...
        if (LastLoc.isNull() || origTL.getBeginLoc() != LastLoc.getBeginLoc()) {
            ...
            else if (type == "cudaError_t")
            {
                RewriteType(tl, "cl_int", HostReplace);
            }
            else {
